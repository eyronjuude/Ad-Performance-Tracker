...
3. **Fetch Trello structure** – Use `get_lists` to retrieve lists; `get_board_labels` and `get_board_members` for workflow steps. Invoke via `call_mcp_tool` with **server** `trello`.
...
Use `call_mcp_tool` with **server** `trello` and the tool name below.
