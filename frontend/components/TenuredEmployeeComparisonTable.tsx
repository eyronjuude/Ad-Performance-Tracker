...
interface TenuredEmployeeComparisonTableProps {
  employees: Employee[];
...
export function TenuredEmployeeComparisonTable({
  employees,
  spendEvaluationKey,
  croasEvaluationKey,
  state,
}: TenuredEmployeeComparisonTableProps) {
  const [phaseOneExpanded, setPhaseOneExpanded] = useState(false);

  if (employees.length === 0) return null;

  return (
    <section aria-labelledby="tenured-comparison-phase-one" className="mb-4">
      <h3 className="mb-4">
        <button
          type="button"
          id="tenured-comparison-phase-one"
          aria-expanded={phaseOneExpanded}
          aria-controls="tenured-comparison-phase-one-panel"
      ...
      {phaseOneExpanded && (
        <div
          id="tenured-comparison-phase-one-panel"
          role="region"
          aria-labelledby="tenured-comparison-phase-one"
